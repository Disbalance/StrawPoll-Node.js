...
exports.getResultVote = function (id,res){
    var count = 0;
          connection.query('SELECT COUNT( * ) AS cnt, options.option FROM (votes INNER JOIN options ON votes.vote = options.id_vote) WHERE options.id_quest ='+id+' GROUP BY vote ORDER BY  "cnt"', function (error, result) {
            var rs = [];
            rs[0] = ['', ''];
            for (i = 0; i < result.length; i++) {
                rs[i + 1]=[];
                var s = result[i].option;
                rs[i+ 1 ][0] = ''+s+'';
