...
exports.createConnection = function () {
    ...
    connection.on('error', function(err) {
        ...
        if (err.code === 'PROTOCOL_CONNECTION_LOST') {
            require('./database').createConnection();
        } else {
            throw err;
        }
    });
...
exports.addPollDB = function(strawpoll,options,res){
    connection.query('INSERT INTO strawpoll SET ?',strawpoll, function(err, result) {
        ...
        res.statusCode = 200;
        res.end("http://localhost:3000/"+String(id));
    });
};
...
function addIpDB(data,vote,res){
    ...
    connection.query('SELECT id_quest FROM users WHERE ip_adress ='+ip, function (error, result) {
        ...
        if (add == true) {
            ...
            res.statusCode = 200;
            res.end('Вы уже голосовали на данный вопрос. Пожалуйста перейдите на "Результаты"');
        }
...
exports.getResultVote = function (id,res){
    ...
    connection.query('SELECT options.id_vote,options.option FROM options WHERE id_quest ='+id,function(error, result) {
        ...
        connection.query('SELECT COUNT( * ) AS cnt, vote FROM votes WHERE votes.id_quest =' + id + ' GROUP BY vote ORDER BY vote', function (error, result) {
            if (result == 0){
                res.statusCode = 202;
            ...
            var k = 0;
            for (i = 0; i < temp.length; i++) {
                if (temp[i].id_vote == result[k].vote) {
                    rs[i + 1][1] = result[k].cnt;
                    count += result[k].cnt;
